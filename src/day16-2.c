...
int main() {
    ...
    iter(i, grid.height * 2 + grid.width * 2) {
        iter(r, grid.height) {
        ...
        Direction start_dir =
            i < grid.height ? right :
            i < grid.height + grid.width ? up :
            i < grid.height * 2 + grid.width ? left :
            down;
        
        Position start_pos =
            i < grid.height ? (Position) { .r = i, .c = 0 } :
            i < grid.height + grid.width ? (Position) { .r = grid.height - 1, .c = i - grid.height } :
            i < grid.height * 2 + grid.width ? (Position) { .r = grid.height * 2 + grid.width - i, .c = grid.width - 1 } :
            (Position) { .r = 0, .c = grid.height * 2 + grid.width * 2 - i };

        trace_beam((Beam) { .direction = start_dir, .position = start_pos });
        ...
        ans = energised > ans ? energised : ans;
    }
